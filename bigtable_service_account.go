...
func main() {
	...
	pathToKeyFile := os.Getenv("GOOGLE_APPLICATION_CREDENTIALS")
	if pathToKeyFile == "" {
		log.Fatalf("please set the GOOGLE_APPLICATION_CREDENTIALS " +
			"environment variable to the location of your " +
			"Service Account credentials JSON file")
	}
	jsonKey, err := ioutil.ReadFile(pathToKeyFile)
	if err != nil {
		log.Fatalf("Could not read credentials file at %s: %v", pathToKeyFile, err)
	}

	config, err := google.JWTConfigFromJSON(jsonKey, bigtable.AdminScope) // or bigtable.Scope etc...
	if err != nil {
		log.Fatalf("Could not generate JWT from credentials file at %s: %v", pathToKeyFile, err)
	}
